...
class ProposalBranch(nn.Module):
    def __init__(self, in_channels, proposal_channels):
        ...
        self.kernel_conv = nn.Sequential(
            Unit1D(in_channels=proposal_channels * 2,
                   output_channels=proposal_channels * 2,
                   kernel_shape=1,
                   activation_fn=None),
            nn.GroupNorm(32, proposal_channels * 2),
    ...
    def apply_gaussian_kernel(self, feature, stamp, sigma=0.5):
        # stamp shape is [b, t, 2]
        # feature shape is [b, c, t]
        b, c, t = feature.size()
        _, s_t, _ = stamp.size()
        feature = feature.view(c, t)

        idx_list = torch.split(stamp, 1, dim=2)
        start_range = torch.arange(t, dtype=torch.float).expand(s_t, t).cuda().detach()  # s_t t
        end_range = torch.arange(t, dtype=torch.float).expand(s_t, t).cuda().detach()

        start_idx = idx_list[0].view(s_t, 1).float()  # s_t 1
        end_idx = idx_list[1].view(s_t, 1).float()  # s_t 1

        start_gauss_kernel = torch.exp(-((start_range - start_idx) ** 2) / (2 * sigma ** 2)).view(s_t, t).permute(1,
                                                                                                                  0)  # b t t
        end_gauss_kernel = torch.exp(-((end_range - end_idx) ** 2) / (2 * sigma ** 2)).view(s_t, t).permute(1,
                                                                                                            0)  # b t t

        start_out = torch.mm(feature, start_gauss_kernel)  # b c//2 t
        end_out = torch.mm(feature, end_gauss_kernel)  # b c//2 t
        out = torch.cat([start_out, end_out], dim=0).view(b, c * 2, s_t)  # b c t

        return out

    def forward(self, feature, frame_level_feature, segments, frame_segments, stamp, frame_stamp):
        # [1, 512, t]

        gaussian_feature = self.apply_gaussian_kernel(feature, stamp)
        gaussian_feature = self.kernel_conv(gaussian_feature)
        fm_short = self.cur_point_conv(feature)
        feature = self.lr_conv(feature)
        # gaussian_feature = self.apply_gaussian_kernel(feature, stamp)
        gaussian_frame_feature = self.apply_gaussian_kernel(frame_level_feature, frame_stamp)
        ...
        prop_roi_feature = self.roi_conv(gaussian_frame_feature)
        prop_feature = torch.cat([prop_roi_feature, gaussian_feature, fm_short], dim=1)
...
class CoarsePyramid(nn.Module):
    ...
    def forward(self, feat_dict, ssl=False):
        ...
        action_frame = self.action_conv(frame_level_feat)
        ...
        for i, feat in enumerate(pyramid_feats):
            ...
            with torch.no_grad():
                ...
                stamp = torch.cat([
                    torch.round(l_segment),
                    torch.round(r_segment)
                ], dim=-1)
            ...
            loc_prop_feat, loc_prop_feat_ = self.loc_proposal_branch(loc_feat, frame_level_feat,
                                                                     segments, frame_segments, stamp, frame_stamp)
        ...
        return loc, conf, prop_loc, prop_conf, center, priors, start, end, \
               start_loc_prop, end_loc_prop, start_conf_prop, end_conf_prop, action_frame
...
class BDNet(nn.Module):
    ...
    def apply_gaussian_kernel(self, feature, stamp, sigma=0.2):
        # stamp shape is [b, t, 2]
        # feature shape is [b, c, t]
        b, c, t = feature.size()
        _, s_t, _ = stamp.size()
        ...
        start_gauss_kernel = torch.exp(-((start_range - start_idx) ** 2) / (2 * sigma ** 2)).view(s_t, t).permute(1, 0) # b t t
        end_gauss_kernel = torch.exp(-((end_range - end_idx) ** 2) / (2 * sigma ** 2)).view(s_t, t).permute(1, 0) # b t t

        start_out = torch.mm(feature, start_gauss_kernel) # b c//2 t
        end_out = torch.mm(feature, end_gauss_kernel) # b c//2 t

        out = (start_out + end_out).view(b, c, s_t)
        return out
    def forward(self, x, proposals=None, ssl=False):
        ...
        if ssl:
            ...
            frame_stamp = torch.cat([
                torch.round(decoded_segments[:, :, :1]),
                torch.round(decoded_segments[:, :, 1:])
            ], dim=-1)
            anchor, positive, negative = [], [], []
            for i in range(3):
                bound_feat = self.apply_gaussian_kernel(top_feat[i], frame_stamp / self.scales[i])

...
def calc_action_loss(action, scores):
    action = torch.tanh(action.permute(0, 2, 1)).mean(-1).view(-1)
    scores = torch.from_numpy(scores[0]).float().cuda()
    loss_action = F.binary_cross_entropy(action,
                                        scores,
                                        reduction='mean')
    return loss_action

def forward_one_epoch(net, clips, targets, scores=None, training=True, ssl=True, action=None):
    clips = clips.cuda()
    ...
    if ssl:
        ...
        loss_l, loss_c, loss_prop_l, loss_prop_c, loss_ct = CPD_Loss(
            ...
            targets)
        try:
            loss_action = calc_action_loss(output_dict['action_frame'], action)
        except:
            import pdb
            pdb.set_trace()
        # try:
        ...
        loss_start = 0.1 + 0.1 * (loss_start_loc_prop + loss_start_conf_prop)
        loss_end = 0.1 + 0.1 * (loss_end_loc_prop + loss_end_conf_prop)
        return loss_l, loss_c, loss_prop_l, loss_prop_c, loss_ct, loss_start, loss_end, loss_action
...
def run_one_epoch(epoch, net, optimizer, data_loader, epoch_step_num, training=True):
    ...
    with tqdm.tqdm(data_loader, total=epoch_step_num, ncols=0) as pbar:
        ...
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags, action) in enumerate(pbar):
            loss_l, loss_c, loss_prop_l, loss_prop_c, \
            loss_ct, loss_start, loss_end, loss_action = forward_one_epoch(
                net, clips, targets, scores, training=training, ssl=False, action=action)
            ...
            cost = loss_l + loss_c + loss_prop_l + loss_prop_c + loss_ct + loss_start + loss_end + 10 * loss_action
    ...
    plog = 'Epoch-{} {} Loss: Total - {:.5f}, loc - {:.5f}, conf - {:.5f}, ' \
           ...
           'IoU - {:.5f}, start - {:.5f}, end - {:.5f}, action - {:.5f}'.format(
        i, prefix, cost_val, loss_loc_val, loss_conf_val, loss_prop_l_val, loss_prop_c_val,
        loss_ct_val, loss_start_val, loss_end_val, loss_action
    )
